#!/usr/bin/python
#
def PRZM_EXAMS_pi(chem_name, noa, scenarios, unit, met, inp, run, exam, MM, DD, YY, CAM_f, DEPI_text, Ar_text, EFF,
                  Drft,
    ...
    shutil.copy(src + "/przm3123_win.exe", src1)
    ########EXAMS files############################
    shutil.copy(src + "/inpsrc1/" + exam, src1)
    shutil.copy(src + "/exams_win.exe", src1)
    ...
    file_update = EXAMS_pi(exam, chem_name, scenarios, met, farm, mw, sol, koc, vp, aem, anm, aqp, tmper, n_ph, ph_out,
                           hl_out)

    #############RUN PRZM-EXAM#####################################
    os.chdir(src1)
    os.rename(src1 + '/' + run, 'PRZM3.RUN')
    src2 = src1 + "/przm3123_win.exe"
    ...
    scenario_name = run.split(".")[0]
    command_exams = "exams_win.exe " + exam + " " + scenario_name + ".PEX " + scenario_name + ".ERR " + scenario_name + ".WRN "
    ...
    file_exclud = P2E_list + PRN_list + PRZ_list + ZTS_list + MSB_list + PLT_list + DVF_list + ['przm3123_lin.exe',
                                                                                                'przm3123_win.exe',
                                                                                                'exams_lin.exe',
                                                                                                'exams_win.exe',
                                                                                                'exams.daf',
