...
def test_przm5(pfac, snowmelt, evapDepth,
               ...
               SpecifyYears, ApplicationTypes, PestAppyDay, PestAppyMon, Rela_a, app_date_type, DepthIncorp,
    ...
    myfile.write(
        '{0}, {1}, {2}, {3}, {4}, {5}, {6}'.format(*[uslek, uslels, uslep, fieldSize, ireg, slope, hydlength]) + "\n")
    ...
    myfile.write(
        '{0}, {1}, {2}, {3}, {4}, {5}'.format(*[1, canopyHoldup, rootDepth, canopyCover, 0, canopyHeight]) + "\n")
    ...
    for i in range(firstyear, lastyear + 1):
        Record_12 = [dayEmerge_text, monthEmerge_text, i, dayMature_text, monthMature_text, (i + addYearM),
                     dayHarvest_text, monthHarvest_text, (i + addYearM + addYearH), 1]
    ...
    for i in range(numHoriz):
        if not SoilProperty_sand:
            ...
            myfile.write('{0}, {1}, {2}, {3:2.1f}, {4:.3f}, {5:.3f}, {6:.3f}, {7:.3f}, {8:.3f}, , , {9:.1f}, ,'.format(
                *Record_19) + "\n")
        else:
            Record_19 = [i + 1, SoilProperty_thick[i], SoilProperty_compartment[i], 0, SoilProperty_bulkden[i],
                         SoilProperty_maxcap[i], SoilProperty_maxcap[i], SoilProperty_mincap[i], SoilProperty_oc[i],
                         SoilProperty_sand[i],
                         SoilProperty_clay[i], 0, bcTemp]
            myfile.write(
                '{0}, {1}, {2}, {3:2.1f}, {4:.3f}, {5:.3f}, {6:.3f}, {7:.3f}, {8:.3f}, {9}, {10}, {11:.1f}, {12}'.format(
                    *Record_19) + "\n")
    ...
    if SpecifyYears == 1:
        ...
        myfile.write(
            "***Record C2  mm,dd,yy, cam, dep, rate, eff, spry, 1cam, 1dep, 1rate, 1eff, 1spry, 2cam, 2dep, 2rate, 2eff, 2spry, " + "\n")
        ...
        for j in range(totalApp):
            ...
            if app_date_type == '0':
                Record_C2_temp = [PestAppyDay[j], PestAppyMon[j], PestAppyYear[j], cam, depi, PestAppyRate[j],
                                  localEff[j], localSpray[j], 1, 4.0, 0, 0, 0, 1, 4.0, 0, 0, 0]
            else:
                now = datetime(1961, int(2), int(16))
                Later = now + timedelta(days=int(Rela_a[j]))
                Later = str(Later)
                Record_C2_temp = [Later[2:4], Later[5:7], PestAppyYear[j], cam, depi, PestAppyRate[j], localEff[j],
                                  localSpray[j], 1, 4.0, 0, 0, 0, 1, 4.0, 0, 0, 0]
        ...
        for k in range(firstyear, lastyear + 1):
            for j in range(appNumber_year):
                ...
                if app_date_type == '0':
                    Record_C2_temp = [PestAppyDay[j], PestAppyMon[j], PestAppyYear, cam, depi, PestAppyRate[j],
                                      localEff[j], localSpray[j], 1, 4.0, 0, 0, 0, 1, 4.0, 0, 0, 0]
                else:
                    now = datetime.datetime(1961, int(2), int(16))
                    Later = now + datetime.timedelta(days=int(Rela_a[j]))
                    Later = str(Later)
                    Record_C2_temp = [Later[8:10], Later[5:7], PestAppyYear, cam, depi, PestAppyRate[j], localEff[j],
                                      localSpray[j], 1, 4.0, 0, 0, 0, 1, 4.0, 0, 0, 0]
    ...
    for kk in range(numHoriz):
        if nchem == 1:
            ...
            myfile.write(
                '{0},{0},{1},{2},{2},{1},{3},{3},{1}'.format(*[soilRate[0], 0, soilRate[1], soilRate[2]]) + "\n")
