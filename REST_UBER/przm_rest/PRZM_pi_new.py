...
def PRZM_pi(noa, met, inp, run, MM, DD, YY, CAM_f, DEPI_text, Ar_text, EFF, Drft):
    ...
    shutil.copy(os.path.join(src, "inpsrc1", run), src1)
    shutil.copy(os.path.join(src, "inpsrc1", inp), src1)
    shutil.copy(os.path.join(src, przm_exe), src1)
    # met=string.capitalize(met)
    #    print(met)
    #    met=met.upper()
    #    print(met)
    shutil.copy(os.path.join(src, "inpsrc1", met), src1)
    # print(os.getcwd())
    # print(os.listdir(src1))   #check what files are copied


    src2 = os.path.join(src1, przm_exe)
