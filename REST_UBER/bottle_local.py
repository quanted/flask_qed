...
def exams_rest(jid):
    try:
        ...
        result = exams_pi_win.exams_pi(chem_name, scenarios, met, farm, mw, sol, koc, vp, aem, anm, aqp, tmper, n_ph,
                                       ph_out, hl_out)
        return {'user_id': 'admin', 'result': result, '_id': jid}
    except Exception, e:
        return errorMessage(e, jid)
...
def pfam_rest(jid):
    try:
        ...
        result = pfam_pi_win.pfam_pi(wat_hl, wat_t, ben_hl, ben_t, unf_hl, unf_t, aqu_hl, aqu_t, hyd_hl, mw, vp, sol,
                                     koc, hea_h, hea_r_t,
                                     noa, dd_out, mm_out, ma_out, sr_out, weather, wea_l, nof, date_f1, nod_out, fl_out,
                                     wl_out, ml_out, to_out,
                                     zero_height_ref, days_zero_full, days_zero_removal, max_frac_cov, mas_tras_cof,
                                     leak, ref_d, ben_d,
...
def przm_exams_rest(jid):
    try:
        ...
        result = PRZM_EXAMS_pi_win.PRZM_EXAMS_pi(chem_name, noa, scenarios, unit, met, inp, run, exam, MM, DD, YY,
                                                 CAM_f, DEPI, Ar, EFF, Drft,
